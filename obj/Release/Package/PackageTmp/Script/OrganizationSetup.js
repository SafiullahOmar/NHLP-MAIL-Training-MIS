...
function ShowData() {
    $.ajax({
        type: "POST",
        url: "OrganizationSetup.aspx/GetFormDetail",
        data: '',
        dataType: "json",
        ...
        success: function (data) {
            ...
            $.each(data.d, function (i, x) {
                var row = "<tr ><td style='text-align:center;'>" + count + "</td>" +
                   ...
                   (x.Edit == true ? "<td><a onclick='EditDetail(this)' class='btn-sm btn-blue edit' href='#'>Edit</a><span style='display:none'>" + x.OrganizationID + "</span><span style='display:none'>" + x.Name + "</span><span style='display:none'>" + x.NameLocal + "</span></td>" : "<td></td>") +
                "</tr>";
                $table += row;
                count++;
            });
...
function Save() {
    ...
    $.ajax({
        type: "post",
        url: "OrganizationSetup.aspx/Save",
        data: jsonObject,
        datatype: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            if (data.d == '0') {
                toastr.success("Organization information is Successfully saved !", "TRA-MIS: Component !", { progressBar: !0 });
                swal({
                    title: "Success Alert!",
                    text: "The Organization information is saved successfully.<br/> THANK YOU..!",
                    type: "success",
                    html: true
                });
                //Clear components
                ...
                swal({
                    title: "STOP!",
                    text: "This organization has alreday exist.<br/>Please review the information.",
                    type: "error",
                    html: true
                });
            } else {
...
function Update() {
    ...
    var a = {};
    a.OrganizationID = $('#hfOrganizationID').val();
    a.Name = $('#txtName').val();
    a.NameLocal = $('#txtNameLocal').val();
    ...
    $.ajax({
        type: "post",
        url: "OrganizationSetup.aspx/Update",
        data: jsonObject,
        datatype: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            if (data.d == '0') {
                toastr.success("Organization information is Successfully updated !", "TRA-MIS: Organization !", { progressBar: !0 });
                swal({
                    title: "Success Alert!",
                    text: "The Organization information is updated successfully.<br/> THANK YOU..!",
                    type: "success",
                    html: true
                });
                //Clear components
